 ...
 */
export function mapZaiResult(result: ZaiResult): WebSearchSource | undefined {
 ...
 */
export function mapZaiResponse(response: ZaiSearchResponse): WebSearchResult {
...
export class ZaiSearchProvider implements WebSearchProvider {
   ...
   */
  constructor(private readonly resolveOptions: () => ZaiSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (!response.ok) {
      ...
      try {
        const parsed = await response.json() as ZaiError
   ...
   */
  private async apiKey(options: ZaiSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    throw new WebError(
      `ZAI search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-zai config',
